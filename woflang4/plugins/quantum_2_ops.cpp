    ...
    void register_ops(WoflangInterpreter& interp) override {
        ...
        interp.register_op("modexp", [](WoflangInterpreter& interp) {
            if (interp.stack.size() < 3) { 
                std::cout << "modexp: Need 3 ints (a, exp, mod)\n"; 
                return; 
            }
            
            auto m = interp.stack.back(); 
            interp.stack.pop_back();
            
            auto e = interp.stack.back(); 
            interp.stack.pop_back();
            ...
            if (a.type == WofType::Integer && e.type == WofType::Integer && m.type == WofType::Integer) {
                ...
                if (mod == 0) {
                    std::cout << "modexp: Modulus cannot be zero\n";
                    interp.stack.push_back(a);
                    interp.stack.push_back(e);
                    interp.stack.push_back(m);
                    return;
                }
                
                // Basic modular exponentiation for demo
                int64_t res = 1;
                for (; exp > 0; exp--) {
                    res = (res * base) % mod;
                }
