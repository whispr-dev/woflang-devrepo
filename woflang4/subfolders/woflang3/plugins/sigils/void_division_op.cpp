...
class VoidDivisionOpPlugin : public WoflangPlugin {
public:
    void register_ops(WoflangInterpreter& interp) override {
        interp.register_op("void_division", [](WoflangInterpreter& interp) {
            if (interp.stack.empty() || 
                ((interp.stack.back().type == WofType::Integer || interp.stack.back().type == WofType::Double) &&
                ...
                std::cout << "⦰  The void divides all. Stack is erased. 'Beware division by zero.'\n";
                interp.clear_stack();
            } else {
...
WOFLANG_PLUGIN_EXPORT void register_plugin(WoflangInterpreter& interp) {
    static VoidDivisionOpPlugin plugin;
