...
public:
    void register_ops(WoflangInterpreter& interp) override {
        ...
        interp.register_op("σ", [](WoflangInterpreter& interp) {
            ...
            for (auto& v : interp.stack) {
                if (v.type == WofType::Double)
                    mean += v.value.type == WofType::Double ? std::get<double>(v.value.value) : v.value.type == WofType::Integer ? static_cast<double>(std::get<int64_t>(v.value.value)) : 0.0, n++;
            ...
            double sumsq = 0.0;
            for (auto& v : interp.stack) {
                double x = v.type == WofType::Double ? v.value.type == WofType::Double ? std::get<double>(v.value.value) : v.value.type == WofType::Integer ? static_cast<double>(std::get<int64_t>(v.value.value)) : 0.0 :
        ...
        interp.register_op("Δ", [](WoflangInterpreter& interp) {
            ...
            double da = a.type == WofType::Double ? a.value.type == WofType::Double ? std::get<double>(a.value.value) : a.value.type == WofType::Integer ? static_cast<double>(std::get<int64_t>(a.value.value)) : 0.0 :
                        a.type == WofType::Integer ? static_cast<double>(std::get<int64_t>(a.value)) : 0.0;
            double db = b.type == WofType::Double ? b.value.type == WofType::Double ? std::get<double>(b.value.value) : b.value.type == WofType::Integer ? static_cast<double>(std::get<int64_t>(b.value.value)) : 0.0 :
