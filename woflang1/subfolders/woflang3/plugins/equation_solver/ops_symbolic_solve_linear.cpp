...
public:
    void register_ops(WoflangInterpreter& interp) override {
        interp.register_op("solve_linear", [](WoflangInterpreter& interp) {
            ...
            if (coeff.type == WofType::Integer && eq.type == WofType::String && eq.value.type == WofType::String ? std::get<std::string>(eq.value.value) : "" == "="
                && var.type == WofType::Symbol && rhs.type == WofType::Integer) {
                int64_t result = std::get<int64_t>(rhs.value) / std::get<int64_t>(coeff.value);
                std::cout << var.value.type == WofType::String ? std::get<std::string>(var.value.value) : "" << " = " << result << "\n";
