...
if tokens[0] == '若':  # Check if the command is a conditional
    if len(tokens) < 4:
        print(f"Error: Not enough tokens for a conditional statement: {tokens}")
        return variables  # Skip this command if it's malformed
    
    condition_var = get_value(tokens[1], variables)
    condition_operator = tokens[2]
    condition_value = get_value(tokens[3], variables)

    print(f"Evaluating condition: {condition_var} {condition_operator} {condition_value}")  # Debug print

    # Ensure that the tokens list has enough elements for '若 ... 则 ... 另 ...'
    if '则' in tokens and '另' in tokens:
        ...
        if evaluate_condition(condition_var, condition_operator, condition_value):
            ...
            var_name = true_assignment[0]  # Ensure there's enough data for assignment
            variables[var_name] = get_value(true_assignment[2], variables)
        else:
            false_assignment = tokens[else_index + 1:]
            var_name = false_assignment[0]  # Ensure there's enough data for assignment
            variables[var_name] = get_value(false_assignment[2], variables)
    else:
        print(f"Error: Conditional statement is missing '则' or '另': {tokens}")
        return variables  # Skip this comman
...
def evaluate_condition(var, operator, value):
    # Ensure both var and value are not None
    if var is None or value is None:
        print(f"Error: Cannot compare {var} with {value}. One of them is None.")
        return False

    if operator == '大':  # Replacing '>' with custom symbol
        return var > value
    elif operator == '小':  # Replacing '<' with custom symbol
        return var < value
    elif operator == '卜':  # Replacing '==' with custom symbol
        return var == value
    elif operator == '丨':  # Replacing '!=' with custom symbol
        return var != value
    elif operator == '走':  # Replacing '>=' with custom symbol
        return var >= value
    elif operator == '夊':  # Replacing '<=' with custom symbol
        return var <= value
    else:
        return False
...
try:
    ...
    print(f"Commands loaded: {commands}")  # Check the commands
