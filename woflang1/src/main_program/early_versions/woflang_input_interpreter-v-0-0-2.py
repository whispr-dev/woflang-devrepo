...
def interpret_command(command, variables):
    ...
    if tokens[1] == '門':
        ...
        if len(tokens) == 3:
            ...
            variables[var_name] = operations[op](operand1, operand2) if operand1 is not None and operand2 is not None else None
        ...
        if '则' in tokens and '另' in tokens:
            ...
            if evaluate_condition(condition_var, condition_operator, condition_value):
                ...
                variables[var_name] = get_value(tokens[then_index + 2], variables)
            else:
                var_name = tokens[else_index + 1]
                variables[var_name] = get_value(tokens[else_index + 2], variables)
    return variables
