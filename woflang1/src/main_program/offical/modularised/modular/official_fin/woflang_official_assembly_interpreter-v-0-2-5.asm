...
ERROR_UNRECOGNIZED_COMMAND:
    ...
    CALL REPORT_ERROR
    JMP NEXT_COMMAND  ; Instead of halting, move to the next command to continue execution
...
PARSE_COMMAND:
    MOV R5, COMMAND_TABLE_BASE  ; Base address of command lookup table
    MOV R6, 0

COMMAND_LOOKUP:
    CMP [R5 + R6], R0  ; Compare current command with table entry
    JEQ EXECUTE_COMMAND
    ADD R6, 4
    JMP COMMAND_LOOKUP

EXECUTE_COMMAND:
    ; Execute the command using its associated function pointer
    MOV R7, [R5 + R6 + 1]  ; Function pointer associated with the command
    CALL [R7]
    RET
