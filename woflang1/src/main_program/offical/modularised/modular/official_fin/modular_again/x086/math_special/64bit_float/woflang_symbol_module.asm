; `woflang_symbol_module.asm` main translation core for woflang x086 64bit float.
...
global BINARY_TO_FUZZY_SYMBOL_HANDLER, FUZZY_TO_BINARY_SYMBOL_HANDLER, IMPLY_SYMBOL HANDLER, NIMPLY_SYMBOL_HANDLER; fuzzy logic
global NULL_SYMBOL_HANDLER, INHIBITION_XY_SYMBOL_HANDLER, BUFFER_SYMBOL_HANDLER, INHIBITION_SYMBOL_HANDLER  ; fuzzy logic 1
...
SUB_VECTOR_SYMBOL_HANDLER:
    ; Call the Vector Subtraction routine from mth.asm
    call SUB_VECTOR_ROUTINE
    ret

DOT_PRODUCT_SYMBOL_HANDLER:
    ; Call the Dot Product routine from mth.asm
    call DOT_PRODUCT_ROUTINE
    ret

MATRIX_DETERMINANT_SYMBOL_HANDLER:
    ; Call the Matrix Determinant routine from mth.asm
    call MATRIX DETERMINANT_ROUTINE
    ret

MATRIX_INVERSE_SYMBOL_HANDLER:
    ; Call the Matrix Inversion routine from mth.asm
    call MATIX_INVERSE_ROUTINE
    ret

MATRIX_DETERMINANT_3X3_SYMBOL_HANDLER:
    ; Call the MAtrix Determinant 3x3 routine from mth.asm
    call MATRIX_DETERMIMNANT_3X3_ROUTINE
    ret

BINARY_TO_FUZZY_CONV_SYMBOL_HANDLER:
    ; Call the Bin 2 Fuzz routine from lgc.asm
    call BINARY_TO_FUZZY_ROUTINE
    ret

FUZZY_TO_BINARY_CONV_SYMBOL_HANDLER:
    ; Call the Fuzz 2 Bin routine from lgc.asm
    call FUZZY_TO_BINARY_ROUTINE
    ret

IMPLY_SYMBOL_HHANDLER:
    ;  Call the Imply routine from lgc.asm
    call IMPLY_ROUTINE
    ret

NIMPLY_SYMBOL_HANDLER:
    ; Call the Nimply routine from lgc.asm
    call NIMPLY_ROUTINE
    ret

NULL_SYMBOL_HANDLER:
    ; Call the Null routine from lgc.asm
    call NULL_ROUTINE
    ret

INHIBITION_SYMBOL_HANDLER:
    ; Call the Inhibition routine from lgc.asm
    call INHIBITION_ROUTINE
    ret
BUFFER_SYMBOL_HANDLER:
    ; Call the Buffer routine from lgc.asm
    call BUFFER_ROUTINE
    ret                     ; No operation needed

INHIBITION_XY_SYMBOL_HANDLER:
    ; Call the Inhibiton routine from lgc.asm    
    call INHIBITION_XY_ROUTINE
    ret

EQUIVALENCE_SYMBOL_HANDLER:
    ; Call the Equivalence routine from lgc.asm
    call EQUIVALENCE_ROUTINE
    ret

IDENTITY_SYMBOL_HANDLER:
    ; Call the Identity routine from lgc.asm
