...
bool loadDictionary(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (!(iss >> symbol >> commandName)) {
            std::cerr << "Error: Invalid dictionary format in line: " << line << std::endl;
            continue;
        }

        // Map the symbol to the corresponding command function
        // You will need to have a function pointer or some mechanism to map the string to a function.
        if (commandName == "cmdAdd") {
            commandMap[symbol] = cmdAdd;
        } else if (commandName == "cmdSub") {
            commandMap[symbol] = cmdSub;
        }
        // Continue this pattern for all possible commands
    }
