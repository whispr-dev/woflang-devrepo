# v0.0.4 with hopefully both fixes ansd refer to script [w/ loop]
...
def rule124_next_row(current_row):
    next_row = [0] * len(current_row)  # Initialize the next row

    for i in range(1, len(current_row) - 1):
        left, center, right = current_row[i - 1], current_row[i], current_row[i + 1]

        # Calculate next state based on Rule 124
        if (left, center, right) == (1, 1, 1):
            ...
            next_row[i] = 1
        elif (left, center, right) == (0, 1, 0):
            next_row[i] = 1
        elif (left, center, right) == (0, 0, 1):
            next_row[i] = 0
        elif (left, center, right) == (0, 0, 0):
            next_row[i] = 0
