...
def update_grid(grid, base_growth_prob, base_decay_prob):
    new_grid = grid.copy()
    size = grid.shape[0]

    for i in range(size):
        for j in range(size):
            growth_prob = base_growth_prob * random.uniform(0.8, 1.2)  # Add variability to growth
            decay_prob = base_decay_prob * random.uniform(0.8, 1.2)    # Add variability to decay
...
grid_size = 250          # Size of the grid
sapling_growth_prob = 0.025  # Lower probability of sapling growing into a stunted tree
tree_decay_prob = 0.35    # Lower probability of stunted tree decaying back to empty
initial_sapling_prob = 0.4 # Initial probability of each cell starting as a sapling
num_steps = 250             # Number of steps to run the CA
