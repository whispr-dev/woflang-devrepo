...
//
// where the fuck did this ccome from??
//
struct tsc_seed
{
uint64_t _rdtsc() {
    uint32_t low, high;
    asm volatile("mrrc p15, 0, %0, %1, c14" : "=r"(low), "=r"(high));  // 16/37?
...
// simple RSG based on modern compler employing ASLR to protect from buffer overflow
// (i.e. Address Space Layout Randomization). allowing generation of random
// numbers without using any libraries: (again, slow but seeds are normally gen v infreq.)
struct random_address_space_seed
{
/* uint64_t _gaslr() {
    uint32_t low, high;
    asm volatile("mrrc p15, 0, %0, %1, c14" : "=r"(low), "=r"(high));  // 16/37?
...
int main() {
    ...
    result_type operator()()
	{
	    ...
	    std::random_address_space rs;
		   if (sizeof(result_type) > sizeof(std::random_address_space::result_type))
